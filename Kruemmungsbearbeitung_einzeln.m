...
Bereich=50; % gibt den Bereich an, in dem um ein gefundenes Extrema keine fehlende Markierung gefunden werden darf, damit das Extrema anerkannt wird
 ...
 try
    if n==1
     if Extrema(n,1)>0
        ...
        Extrema(n,5)=find(fas_kamera_bv1_LIN_01_02_HorKruemm_average_t00(1:Extrema(n,2))>=-Tb,1,'last')+1; %letze "+1" dient dazu den Bereich zu verkleinern damit wirklich nur Messpunkte verwendet werden deren Krümmung <=0.0001 ist
     end
...
for n=1:size(Extrema,1)
 try
     if n==1
        if  Extrema(n,1)>0
            Extrema(n,7)=find(fas_kamera_bv1_LIN_01_02_HorKruemm_average_t00(Extrema(n,2):Extrema(n+1,2))<=Tb,1,'first')+Extrema(1,2)-1-1;%letze "-1" dient dazu den Bereich zu verkleinern damit wirklich nur Messpunkte verwendet werden deren Krümmung <=0.0001 ist
        else
